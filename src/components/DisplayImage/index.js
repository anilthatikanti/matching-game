...
class DisplayImage extends Component {
  ...
  selectedImage = imageUrl => {
    ...
    if (imagesList[randomNumber].imageUrl === imageUrl) {
      ...
      this.setState({isSelect: false, seconds: 0})
    }
  }
  ...
  render() {
    const {tabId, seconds, randomNumber, score, isSelect} = this.state
                ...
                <ul className="tabStyle">
                  {tabsList.map(each => (
                    <TabItems
                      ...
                      className={tabId === each.tabId}
                    />
                  ))}
                </ul>
              </div>
              <div className="thumbnailContainer">
                <ul className="thumbnailStyle">
                  {filterImages.map(eachImage => (
                    <ThumbnailItems
                      thumbnailDetails={eachImage}
                      key={eachImage.id}
                      selectedImage={this.selectedImage}
                      randomImage={imagesList[randomNumber].imageUrl}
                    />
                  ))}
                </ul>
              </div>
